...
TEST(StateMachine, RegularFSM)
{
    ...
    ASSERT_EQ(S::Idle, static_cast<S>(fsm.GetCurrentState()));
    ...
    fsm.Subscribe("test", [&](S newState){
        switch (newState)
        {
            case S::Idle:
                if (cnt > 0) ASSERT_NO_THROW(fsm.ChangeState(T::End));
                ++cnt;
                break;
            default:
                break;
...
TEST(StateMachine, GetCurrentState)
{
    StateMachine fsm;
    std::thread runner(&StateMachine::Run, &fsm);

    fsm.ChangeState(T::InitDevice);
    fsm.ChangeState(T::Automatic);

    auto state = fsm.GetCurrentState();

    while (S::DeviceReady != static_cast<S>(state))
    {
        state.Unlock();
        state = move(fsm.GetCurrentState());
    }

    fsm.ChangeState(T::ResetDevice, state);

    fsm.ChangeState(T::Automatic);
    fsm.ChangeState(T::End);
