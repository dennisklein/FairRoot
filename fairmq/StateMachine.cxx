...
auto StateMachine::Run() -> void
{
    ...
    fCallbacks.Emit<StateChange>(fErrorState);
    LOG(STATE) << "Entering initial " << fState << " state";
    fCallbacks.Emit<StateChange>(fState);
...
auto StateMachine::ChangeState(StateTransition transition, unique_lock<mutex>&& lock) -> void
{
    assert(lock.mutex() == &fMutex);

    if (transition == StateTransition::ErrorFound)
    {
        auto nextState = Transition(fErrorState, transition);
    ...
    {
        auto nextState = Transition(fState, transition);
        fNextStates.push_back(nextState);
        lock.unlock();
    ...
    {
        auto nextState = Transition(fNextStates.back(), transition);
        fNextStates.push_back(nextState);
        lock.unlock();
        fNewState.notify_one();
    }
